...
@test "Fail2Ban is running" {
  run _check_if_process_is_running 'fail2ban-server'
  assert_success
}
...
@test "ban ip on multiple failed login" {
  ...
  run _repeat_in_container_until_success_or_timeout 10 "${CONTAINER_NAME}" bash -c "fail2ban-client status postfix-sasl | grep -F '${CONTAINER2_IP}'"
  assert_success
...
@test "restart of Fail2Ban" {
  ...
  _run_until_success_or_timeout 10 _check_if_process_is_running 'fail2ban-server'
}
